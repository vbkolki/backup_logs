...
#_NODETOOL=$(/home/vishwa/cassandra-trunk/src/java/org/apache/cassandra/tools/nodetool)
...
then
	...
	sudo mkdir -p "$_BACKUP_SCHEMA_DIR"
fi
...
then
	...
	sudo mkdir -p "$_BACKUP_SNAPSHOT_DIR"
fi
...
sudo cqlsh -e "DESC KEYSPACES" |perl -pe 's/\e([^\[\]]|\[.*?[a-zA-Z]|\].*?\a)//g' | sed '/^$/d' > Keyspace_name_schema.cql
...
then
	...
	sudo mkdir -p $_BACKUP_SCHEMA_DIR/$i
fi
done
...
do
#gksu nautilus /backup/2016-01-04/SCHEMA/vishwa/vishwa_schema-2016-01-04-1229-01.cql
...
nodetool snapshot -t $_SNAPSHOT_NAME
...
then
	...
	sudo mkdir -p $_BACKUP_SNAPSHOT_DIR/$i
...
## Triming _DATA_DIR
	...
	sudo cp -prvf "$SNP_VAR" "$_BACKUP_SNAPSHOT_DIR$_SNP_PATH_TRIM";
